...
use crate::im_common::{IMMessage, MsgType, UsrMessage};
use bincode;
use std::env;
            ...
            let _ = match conns[i].conn.read(&mut data) {
                ...
                Ok(m) => {
                    if m > 0 {
                        ...
                        let m1: IMMessage = bincode::deserialize(&data).unwrap();
                        match m1.mtype {
                            MsgType::MsgJoin => {
...
fn main() {
    ...
    let _ = match TcpListener::bind("0.0.0.0:".to_string() + args[1].as_str()) {
        ...
        Ok(m) => {
            println!("Starting Instant Messaging Server at TCP port {}...", args[1]);
            tcp_listener = m;
        }
    };
